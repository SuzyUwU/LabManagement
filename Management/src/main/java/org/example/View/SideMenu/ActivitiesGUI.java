...
public class ActivitiesGUI {
    ...
    public static void showActivities(List<Activity> allActivities, User user) {
        ...
        filterDropdown.addActionListener(e -> {
            ...
            if ("Joined Activities".equals(selectedFilter)) {
                filteredActivities = allActivities.stream()
                        .filter(activity -> activity.hasJoinedActivity(user.getUserId(), activity.getActivityId()))
        ...
        // Initialize the activity list with all activities
        ...
        cancelActivityButton.addActionListener(e -> {
            ...
            if (selectedIndex != -1) {
                ...
                if (confirmation == JOptionPane.YES_OPTION) {
                    boolean success = selectedActivity.cancelActivity(user.getUserId(), selectedActivity.getActivityId());
                    if (success) {
                        JOptionPane.showMessageDialog(frame, "Successfully canceled the activity!");
                        filterDropdown.setSelectedItem(filterDropdown.getSelectedItem()); // Refresh list
                    } else {
